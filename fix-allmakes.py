    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground font-sans overflow-x-hidden">
      ...
      <div className="border-b border-border bg-card">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <button 
            onClick={() => navigate(-1)}
            className="flex items-center text-sm font-medium text-muted-foreground hover:text-primary transition-colors cursor-pointer group"
          >
        ...
        >
          {filteredBrands.map((brand) => (
            <button
              ...
              className="group relative flex flex-col items-center justify-center p-4 rounded-2xl border bg-card border-border hover:border-primary/50 hover:bg-primary/5 transition-all cursor-pointer shadow-sm hover:shadow-md text-center"
            >
              <div className="w-12 h-12 sm:w-14 sm:h-14 mb-3 rounded-xl flex items-center justify-center overflow-hidden bg-background shadow-inner">
                <img 
                  ...
                  onError={(e) => {
                    ...
                    e.currentTarget.parentElement!.innerHTML = '<span class="text-xl font-black text-primary">' + brand.name.slice(0,2).toUpperCase() + '</span>';
                  }}
                />
              </div>
...
with open('src/pages/AllMakes.tsx', 'w') as f:
    f.write(code.strip())
