...
content = re.sub(
    r\"const \[activeTab, setActiveTab\] = useState<\'safety\' \| \'parts\'>\(\'parts\'\);\",
    \"const [activeTab, setActiveTab] = useState<'parts' | 'recalls' | 'investigations' | 'complaints'>('parts');\",
    content
)
...
content = re.sub(
    r'<div className=\"flex items-center gap-2 mb-8 bg-muted/50 p-1\.5 rounded-2xl w-fit mx-auto sm:mx-0\">.*?</NHTSASafetyHub>',
    tabs_ui.strip(),
    content,
    flags=re.DOTALL
)
